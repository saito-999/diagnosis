...
function renderResult(st){
  ...
  const tableCard = el('section', {class:'card'}, el('div', {class:'card-inner stack'}, [
    ...
    el('div', {class:'note'}, `レアリティ凡例：${rarityLegendText()}`),
  ]));
  ...
  const textsCard = el('section', {class:'card'}, el('div', {class:'card-inner stack'}, [
    ...
    ...(r?.phaseTexts ?? []).map(pt => el('details', {class:'details'}, [
      ...
      el('div', {class:'details-inner stack'}, (pt.blocks ?? []).map(b => el('div', {class:'stack'}, [
        el('div', {class:'note'}, String(b.title ?? '')),
  ...
  return el('div', {class:'stack'}, [topCard, tableCard, textsCard, footerCard]);
}
