...
async function tryCalcResultKeys(answersNormalized, phasesRes){
  try{
    const mod = await import("./result_key_logic.js");
    if (typeof mod.calcResultKeys === "function"){
      const r = await mod.calcResultKeys({ answers: answersNormalized });
      if (r && r.patternKeysByPhase) return r.patternKeysByPhase;
    }
  }catch{}

  const fromContrib =
    (phasesRes && (phasesRes.patternKeysByPhase || phasesRes.pattern_keys_by_phase)) || null;
  if (fromContrib) return fromContrib;
...
async function computeResult(){
  ...
  const patternKeysByPhase = await tryCalcResultKeys(answersNormalized, phasesRes);
