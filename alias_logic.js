// alias_logic.js
// 契約: 仕様書_別紙_異名選出ロジック_LOCKED_v7.md に準拠（完全決定論 / 乱数なし）
//
// 公開I/O（本紙側が呼ぶ想定）:
//   - export function calcAlias(answers, rarityOverall)
//       answers: number[20]（1..5） ※正規化済み
//       rarityOverall: "C"|"U"|"R"|"E"|"M"|"Lg"|"Sg"
//     戻り値: { aliasOverall, aliasCategoryOverall, aliasAssetOverall, aliasId, assetCandidates, debug? }
//
// NOTE:
// - phase（弱→強/平/強→弱）サブ分類は別紙に存在するが、本紙側の呼び出しI/Oでは未指定のため、
//   この実装では phaseTrend 未指定時は "flat" を使用する。
//   （将来、本紙から phaseTrend を渡す場合は calcAlias の第3引数で受け取れる）
...
export const ALIAS_MASTER = {
  ...
  // Sg は main/sub を 1候補として扱う（表示は main + "\n　" + sub）
  Sg: {
    distance: [
...
// ------------------------------------------------------------
const phase_w_alias = [
  { matching:0.8, firstMeet:1.0, date:0.4, relationship:0.2, marriage:0.0 },
  { matching:0.9, firstMeet:0.8, date:0.4, relationship:0.2, marriage:0.0 },
  { matching:0.7, firstMeet:0.8, date:0.6, relationship:0.2, marriage:0.0 },
  { matching:0.5, firstMeet:1.0, date:0.4, relationship:0.2, marriage:0.0 },
  { matching:0.8, firstMeet:0.9, date:0.4, relationship:0.2, marriage:0.0 },
  { matching:0.2, firstMeet:0.5, date:1.0, relationship:0.7, marriage:0.4 },
  { matching:0.1, firstMeet:0.5, date:0.8, relationship:1.0, marriage:0.7 },
  { matching:0.1, firstMeet:0.3, date:0.7, relationship:1.0, marriage:0.9 },
  { matching:0.8, firstMeet:0.8, date:0.5, relationship:0.2, marriage:0.0 },
  { matching:0.1, firstMeet:0.2, date:0.4, relationship:0.9, marriage:1.0 },
  { matching:0.0, firstMeet:0.2, date:0.6, relationship:0.9, marriage:0.9 },
  { matching:0.4, firstMeet:0.7, date:0.6, relationship:0.5, marriage:0.3 },
  { matching:0.0, firstMeet:0.2, date:0.6, relationship:1.0, marriage:0.9 },
  { matching:0.0, firstMeet:0.2, date:0.6, relationship:1.0, marriage:0.9 },
  { matching:0.0, firstMeet:0.1, date:0.5, relationship:1.0, marriage:1.0 },
  { matching:0.0, firstMeet:0.0, date:0.3, relationship:0.8, marriage:1.0 },
  { matching:0.0, firstMeet:0.1, date:0.3, relationship:0.8, marriage:1.0 },
  { matching:0.0, firstMeet:0.1, date:0.4, relationship:0.9, marriage:1.0 },
  { matching:0.0, firstMeet:0.0, date:0.3, relationship:0.8, marriage:1.0 },
  { matching:0.1, firstMeet:0.2, date:0.5, relationship:0.9, marriage:1.0 },
];
...
// app.js が import { calcAlias } を使う想定に合わせる（本紙: 別紙I/Oは「合わせる」方針）
export function calcAlias(answers, rarityOverall, phaseTrend = "flat") {
  return computeAliasOverall(answers, rarityOverall, phaseTrend);
}

// 後方互換（旧名が使われている場合）
export const computeAlias = computeAliasOverall;
