...
export function aliasIdFromAliasOverall(rarityOverall, aliasOverall) {
  ...
  const firstMeetLine = String(aliasOverall).split("\n")[0] || "";
  const idx = firstMeetLine.indexOf("《");
  return (idx >= 0 ? firstMeetLine.slice(0, idx) : firstMeetLine).trim();
}
...
export function buildAliasAssetCandidates(rarityOverall, aliasId) {
  ...
  const firstMeet = preferGif ? `${base}.gif` : `${base}.png`;
  const second = preferGif ? `${base}.png` : `${base}.gif`;
  return [firstMeet, second, `_default.png`];
}
